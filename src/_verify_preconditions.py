...
import _ischarstr
import _isrealvector
import _isrealscalar
import _isintegerscalar
import _isnumvec
import _islogicalscalar
import pdb
...
def verify_preconditions(fun, x0, options):
    ...
    if not (_ischarstr.ischarstr(fun) or callable(fun)):
        raise ValueError("fun should be a function handle.")

    if not _isrealvector.isrealvector(x0)[0]:
        raise ValueError("x0 should be a real vector.")
    # pdb.set_trace()
    if "MaxFunctionEvaluations" in options:
        if not (_isintegerscalar.isintegerscalar(options["MaxFunctionEvaluations"]) and options[
            "MaxFunctionEvaluations"] > 0):
            raise ValueError("MaxFunctionEvaluations should be an integer scalar.")

    if "num_blocks" in options:
        if not (_isintegerscalar.isintegerscalar(options["num_blocks"]) and options["num_blocks"] > 0):
    ...
    if "expand" in options:
        if not (_isrealscalar.isrealscalar(options["expand"]) and options["expand"] >= 1):
            raise ValueError("expand should be a real number greater than or equal to 1.")

    if "shrink" in options:
        if not (_isrealscalar.isrealscalar(options["shrink"]) and options["expand"] > 0 and options["shrink"] < 1):
            raise ValueError("shrink should be a real number in [0, 1).")

    if "reduction_factor" in options:
        if not (_isnumvec.isnumvec(options["reduction_factor"]) and len(options["reduction_factor"]) == 3):
