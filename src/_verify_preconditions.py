...
def verify_preconditions(fun, x0, options):
    ...
    if not (__init__.ischarstr(fun) or callable(fun)):
    ...
    if "num_blocks" in options:
        if not (__init__.isintegerscalar(options["num_blocks"]) and options["num_blocks"] > 0):
    ...
    if "expand" in options:
        if not (__init__.isrealscalar(options["expand"]) and options["expand"] >= 1):
            raise ValueError("expand should be a real number greater than or equal to 1.")

    if "shrink" in options:
        if not (__init__.isrealscalar(options["shrink"]) and options["expand"] > 0 and options["shrink"] < 1):
            raise ValueError("shrink should be a real number in [0, 1).")

    if "reduction_factor" in options:
        if not (__init__.isnumvec(options["reduction_factor"]) and len(options["reduction_factor"]) == 3):
